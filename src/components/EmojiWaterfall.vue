<template>
  <div id="master">
    <h1 v-for="(item, index) in msg" :key="Math.round(new Date().getTime() / 1000).toString()"
      :style="{ color: '#fff', '--text-index': index + 1 }" :id="`text-${index}`">{{ item }}</h1>
  </div>
</template>
...
const runSnow = () => {
  ...
  if (!isStop.value && getRandomInitInclusive(0, 5) === 5 && snowLeft > 50) {
    ...
    snow.style.setProperty('--dm-scroll-height', `${height + 60}px`)
    snow.style.setProperty('--dm-scroll-ms', `${sinkTime}s`)
  }
...
onMounted(() => {
  ...
  onTimeText(props.msg)
})

const onTimeText = (msg: string[]) => {
  msg.map((_, index) => {
    setTimeout(() => {
      const documentText = document.getElementById('text-' + index)
      if (documentText) {
        documentText.style.color = '#213547'
        documentText.className = 'text'
      }
    }, index * 3000)
  })
}
